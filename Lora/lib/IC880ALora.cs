...
0xFF, 0xBF, 0xFF, 0xBF, 0xFF, 0xBF, 0xFF, 0xBF, 0xFF, 0xBF, 0xFF, 0xBF, 0xFF, 0xBF, 0xFF, 0xBF }, 8192, 4, 0x20);
    ...
    public override Boolean StartRadio() {
      ...
      this.Sx125xSetup(0, 1, this.radioEnabled[0], RadioType.SX1257, this.radioFrequency[0]);
      ...
      this.RegisterWrite(Registers.IF_FREQ_0, (this.interfaceFrequency[0] << 5) / 15625); /* default -384 */
      this.RegisterWrite(Registers.IF_FREQ_1, (this.interfaceFrequency[1] << 5) / 15625); /* default -128 */
      this.RegisterWrite(Registers.IF_FREQ_2, (this.interfaceFrequency[2] << 5) / 15625); /* default 128 */
      this.RegisterWrite(Registers.IF_FREQ_3, (this.interfaceFrequency[3] << 5) / 15625); /* default 384 */
      this.RegisterWrite(Registers.IF_FREQ_4, (this.interfaceFrequency[4] << 5) / 15625); /* default -384 */
      this.RegisterWrite(Registers.IF_FREQ_5, (this.interfaceFrequency[5] << 5) / 15625); /* default -128 */
      this.RegisterWrite(Registers.IF_FREQ_6, (this.interfaceFrequency[6] << 5) / 15625); /* default 128 */
      this.RegisterWrite(Registers.IF_FREQ_7, (this.interfaceFrequency[7] << 5) / 15625); /* default 384 */

      this.RegisterWrite(Registers.CORR0_DETECT_EN, (this.interfaceEnabled[0]) ? (Byte)SF.DR_LORA_SFMULTI : 0); /* default 0 */
      this.RegisterWrite(Registers.CORR1_DETECT_EN, (this.interfaceEnabled[1]) ? (Byte)SF.DR_LORA_SFMULTI : 0); /* default 0 */
      this.RegisterWrite(Registers.CORR2_DETECT_EN, (this.interfaceEnabled[2]) ? (Byte)SF.DR_LORA_SFMULTI : 0); /* default 0 */
      this.RegisterWrite(Registers.CORR3_DETECT_EN, (this.interfaceEnabled[3]) ? (Byte)SF.DR_LORA_SFMULTI : 0); /* default 0 */
      this.RegisterWrite(Registers.CORR4_DETECT_EN, (this.interfaceEnabled[4]) ? (Byte)SF.DR_LORA_SFMULTI : 0); /* default 0 */
      this.RegisterWrite(Registers.CORR5_DETECT_EN, (this.interfaceEnabled[5]) ? (Byte)SF.DR_LORA_SFMULTI : 0); /* default 0 */
      this.RegisterWrite(Registers.CORR6_DETECT_EN, (this.interfaceEnabled[6]) ? (Byte)SF.DR_LORA_SFMULTI : 0); /* default 0 */
      this.RegisterWrite(Registers.CORR7_DETECT_EN, (this.interfaceEnabled[7]) ? (Byte)SF.DR_LORA_SFMULTI : 0); /* default 0 */
      ...
      this.RegisterWrite(Registers.IF_FREQ_8, (this.interfaceFrequency[8] << 5) / 15625); /* configure LoRa 'stand-alone' modem (IF8) */ /* MBWSSF modem (default 0) */
      if (this.interfaceEnabled[8] == true) {
        this.RegisterWrite(Registers.MBWSSF_RADIO_SELECT, (Byte)this.interfaceChain[8]);
        switch (this.loraBandwidth) {
          case BW.BW_125KHZ: this.RegisterWrite(Registers.MBWSSF_MODEM_BW, 0); break;
          case BW.BW_250KHZ: this.RegisterWrite(Registers.MBWSSF_MODEM_BW, 1); break;
          case BW.BW_500KHZ: this.RegisterWrite(Registers.MBWSSF_MODEM_BW, 2); break;
        }
        switch (this.loraSpreadingFactor) {
          case SF.DR_LORA_SF7: this.RegisterWrite(Registers.MBWSSF_RATE_SF, 7); break;
          case SF.DR_LORA_SF8: this.RegisterWrite(Registers.MBWSSF_RATE_SF, 8); break;
          case SF.DR_LORA_SF9: this.RegisterWrite(Registers.MBWSSF_RATE_SF, 9); break;
          case SF.DR_LORA_SF10: this.RegisterWrite(Registers.MBWSSF_RATE_SF, 10); break;
          case SF.DR_LORA_SF11: this.RegisterWrite(Registers.MBWSSF_RATE_SF, 11); break;
          case SF.DR_LORA_SF12: this.RegisterWrite(Registers.MBWSSF_RATE_SF, 12); break;
        }
        this.RegisterWrite(Registers.MBWSSF_PPM_OFFSET, ((this.loraBandwidth == BW.BW_125KHZ) && ((this.loraSpreadingFactor == SF.DR_LORA_SF11) || (this.loraSpreadingFactor == SF.DR_LORA_SF12))) || ((this.loraBandwidth == BW.BW_250KHZ) && (this.loraSpreadingFactor == SF.DR_LORA_SF12)) ? 1 : 0); /* default 0 */
        this.RegisterWrite(Registers.MBWSSF_MODEM_ENABLE, 1); /* default 0 */
      } else {
        this.RegisterWrite(Registers.MBWSSF_MODEM_ENABLE, 0);
      }

      this.RegisterWrite(Registers.IF_FREQ_9, (this.interfaceFrequency[9] << 5) / 15625);/* configure FSK modem (IF9) */ /* FSK modem, default 0 */
      this.RegisterWrite(Registers.FSK_PSIZE, this.fskSyncWordSize - 1);
      this.RegisterWrite(Registers.FSK_TX_PSIZE, this.fskSyncWordSize - 1);
      ...
      if (this.interfaceEnabled[9] == true) {
        this.RegisterWrite(Registers.FSK_RADIO_SELECT, (Byte)this.interfaceChain[9]);
        this.RegisterWrite(Registers.FSK_BR_RATIO, (Int32)(32000000 / this.fskDatarate)); /* setting the dividing ratio for datarate */
        this.RegisterWrite(Registers.FSK_CH_BW_EXPO, (Byte)this.fskBandwidth);
      ...
      for (Byte i = 0; i < Txgain_lut.size; ++i) { /* Update Tx gain LUT and start AGC */
        ...
        Thread.Sleep(1);
        this.RegisterWrite(Registers.RADIO_SELECT, Txgain_lut.lut[i].mix_gain + (16 * Txgain_lut.lut[i].dac_gain) + (64 * Txgain_lut.lut[i].pa_gain));
        Thread.Sleep(1);
      ...
      if (recieveregister[0] > 0 && recieveregister[0] <= 16) {
        ...
        if ((ifmod == RadioDataType.LoraMulti) || (ifmod == RadioDataType.Lora)) {
          ...
          if (ifmod == RadioDataType.LoraMulti) {
            p.bandwidth = BW.BW_125KHZ; /* fixed in hardware */
          } else {
            p.bandwidth = this.loraBandwidth; /* get the parameter from the config variable */
          }
          ...
          Boolean ppm;
          if ((((p.bandwidth == BW.BW_125KHZ) && ((p.spreadingfactor == SF.DR_LORA_SF11) || (p.spreadingfactor == SF.DR_LORA_SF12))) || ((p.bandwidth == BW.BW_250KHZ) && (p.spreadingfactor == SF.DR_LORA_SF12)))) {
            ppm = true;
          } else {
            ppm = false;
          }
    ...
    public void SetSpreadingFactor(Byte sf) {
      if(sf <= 7) {
        this.loraSpreadingFactor = SF.DR_LORA_SF7;
      } else if(sf <= 8) {
        this.loraSpreadingFactor = SF.DR_LORA_SF8;
      } else if (sf <= 9) {
        this.loraSpreadingFactor = SF.DR_LORA_SF9;
      } else if (sf <= 10) {
        this.loraSpreadingFactor = SF.DR_LORA_SF10;
      } else if (sf <= 11) {
        this.loraSpreadingFactor = SF.DR_LORA_SF11;
      } else {
        this.loraSpreadingFactor = SF.DR_LORA_SF12;
      }
    }
    ...
    private Int32 RegisterRead(LGWRegisters register) {
      ...
      if ((register.BitOffset + register.SizeInBits) <= 8) { /* read one byte, then shift and mask bits to get reg value with sign extension if needed */
        ...
        if (register.SignedInt == true) {
          u = u << (32 - register.SizeInBits); /* left-align the data */
    ...
    private Boolean RegisterWrite(LGWRegisters register, Int32 value) {
      if (register.Equals(Registers.PAGE_REG)) {
        ...
        if ((value & 0x01) != 0) {
          this.SingleSPI((Byte)(0x80 | (Registers.SOFT_RESET.Address & 0x7F)), 0x80);
        }
        return true;
      }
      ...
      if ((register.SizeInBits == 8) && (register.BitOffset == 0)) {
        this.SingleSPI((Byte)(0x80 | (register.Address & 0x7F)), (Byte)value);
        ...
        this.SingleSPI((Byte)(0x80 | (register.Address & 0x7F)), buf[3]);
        ...
        this.MultiSPI((Byte)(0x80 | (register.Address & 0x7F)), buf, size); // write the register in one burst
      } else { // register spanning multiple memory bytes but with an offset
        return false;
      }
      return true;
    ...
    private Boolean RegisterWriteArray(LGWRegisters register, Byte[] value, UInt16 size) {
      ...
      this.MultiSPI((Byte)(0x80 | (register.Address & 0x7F)), value, size); /* do the burst write */
      return true;
    }
    #endregion
    ...
    private void PageSwitch(Byte targetPage) {
      ...
      this.SingleSPI((Byte)(0x80 | (Registers.PAGE_REG.Address & 0x7F)), this.selectedPage);
    }
    ...
    private Boolean LoadFirmware(Firmwaredata fw) {
      ...
      this.RegisterWriteArray(Registers.MCU_PROM_DATA, fw.Data, fw.Size);      /* write the program in one burst */

      this.RegisterRead(Registers.MCU_PROM_DATA);       /* Read back firmware code for check */ /* bug workaround */
